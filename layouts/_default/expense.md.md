...
|--------|--------|------|-------------|
{{ $totalSpent :=0 }}{{ range .Site.Data.expenses }}{{if eq .paid false }}| {{ .purpose }} | {{ .amount }} |{{ .date }} | {{ .explanation }} |{{ $totalSpent := 0 }}|{{ if eq .paid false }}|{{ $totalSpent = add $totalSpent .amount }} | {{ end }} {{ end }}|
